...
{% highlight php %} foreach(PDO::getAvailableDrivers() as $driver){
...
{% highlight php %} $conn->setAttribute(PDO::ATTR\_ERRMODE, PDO::ERRMODE\_WARNING);
...
{% highlight php %} $clientes = $conn->prepare('SELECT id, nome FROM clientes');
...
{% highlight php %} $clientes = $conn->prepare('SELECT id, nome FROM clientes WHERE id = :id AND nome = :nome');
...
{% highlight php %} $clientes = $conn->prepare('INSERT INTO clientes (nome) VALUES (:nome));
